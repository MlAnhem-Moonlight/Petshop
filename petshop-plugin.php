...
function petshop_create_tables() {
    ...
    $sql = "CREATE TABLE IF NOT EXISTS $table_pets (
        ...
        PRIMARY KEY (id),
        KEY user_id (user_id),
        FOREIGN KEY (user_id) REFERENCES {$wpdb->prefix}petshop_users(id) ON DELETE CASCADE
    ) $charset_collate;";
    dbDelta($sql);

    $sql = "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}petshop_suppliers (
        id BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
        name VARCHAR(100) NOT NULL,
        phone VARCHAR(50),
        email VARCHAR(100),
...
function insert_sample_data() {
    ...
    $pets = [
        [
            ...
            'last_checkup' => date('Y-m-d', strtotime('-2 months')),
            'created_at' => current_time('mysql')
        ],
        [
            'user_id' => 2,
        ...
        [
            ...
            'age' => 1,
            'health_status' => 'Good',
            'last_checkup' => date('Y-m-d', strtotime('-3 weeks')),
            'created_at' => current_time('mysql')
        ]
    ];

    foreach ($pets as $pet) {
        $result = $wpdb->insert($wpdb->prefix . 'petshop_pets', $pet);
        if ($result === false) {
            error_log('Failed to insert pet: ' . $wpdb->last_error);
        }
    }
    ...
    $suppliers = [
        [
            'name' => 'Công ty TNHH Thú Cưng Việt',
            'phone' => '0909123456',
            'email' => 'contact@thucungviet.vn',
            'address' => '123 Lê Lợi, Hà Nội',
            'note' => 'Chuyên cung cấp thức ăn chó mèo'
        ],
        [
            'name' => 'Pet World',
            'phone' => '0988123456',
            'email' => 'info@petworld.vn',
            'address' => '456 Nguyễn Trãi, TP.HCM',
            'note' => 'Nhà cung cấp phụ kiện thú cưng'
        ],
        [
            'name' => 'Siêu Thị Chim Cảnh',
            'phone' => '0912345678',
            'email' => 'sieuthichim@gmail.com',
            'address' => '789 Trần Phú, Hải Phòng',
            'note' => 'Chim cảnh và lồng chim'
        ]
    ];

    foreach ($suppliers as $supplier) {
        $result = $wpdb->insert($wpdb->prefix . 'petshop_suppliers', $supplier);
        if ($result === false) {
            error_log('Failed to insert supplier: ' . $wpdb->last_error);
        }
    }
}
...
// Add to cart AJAX handler with stock check
add_action('wp_ajax_petshop_add_to_cart_db', function() {
    global $wpdb;
    // SỬA DÒNG NÀY: Lấy user_id từ session giống như cart.php
    ...
    if ($quantity > $product->stock_quantity) {
        wp_send_json_error('Vượt quá số lượng trong kho!');
    }
    ...
    if (petshop_is_logged_in() && isset($_SESSION['ps_user_role'])) {
        if ($_SESSION['ps_user_role'] === 'admin') {
            ...
            add_submenu_page(
                ...
                function() {
                    petshop_check_auth();
                    require_once PETSHOP_PLUGIN_DIR . 'modules/admin/ordersmanagement.php';
                }
            );
            ...
            add_submenu_page(
                ...
                function() {
                    petshop_check_auth();
                    require_once PETSHOP_PLUGIN_DIR . 'modules/admin/suppliersmanagement.php';
                    petshop_suppliers_page();
                }
            );
...
add_action('wp_ajax_petshop_delete_category', function() {
    check_ajax_referer('petshop_delete_category');
    global $wpdb;
    $id = intval($_POST['id']);
    $result = $wpdb->delete($wpdb->prefix.'petshop_categories', ['id' => $id]);
    if ($result) {
        wp_send_json_success();
    } else {
        wp_send_json_error();
...
add_action('wp_ajax_petshop_edit_category', function() {
    check_ajax_referer('petshop_edit_category');
    global $wpdb;
    $id = intval($_POST['id']);
    $name = sanitize_text_field($_POST['name']);
    $description = sanitize_textarea_field($_POST['description']);
...
add_action('wp_ajax_petshop_add_category', function() {
    check_ajax_referer('petshop_add_category');
    global $wpdb;
    $name = sanitize_text_field($_POST['name']);
    $description = sanitize_textarea_field($_POST['description']);
    $result = $wpdb->insert(
...
add_action('wp_ajax_petshop_save_pet', function() {
    global $wpdb;
    $table = $wpdb->prefix.'petshop_pets';
    $data = [
        ...
        'health_status' => sanitize_text_field($_POST['health_status']),
        'last_checkup' => sanitize_text_field($_POST['last_checkup']),
        'created_at' => current_time('mysql')
    ];
    if (!empty($_POST['pet_id'])) {
        $id = intval($_POST['pet_id']);
        unset($data['created_at']);
        $r = $wpdb->update($table, $data, ['id'=>$id]);
    } else {
...
add_action('wp_ajax_petshop_get_pet', function() {
    global $wpdb;
    $id = intval($_POST['id']);
    $pet = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}petshop_pets WHERE id=$id", ARRAY_A);
    if ($pet) wp_send_json_success($pet);
    else wp_send_json_error();
});

add_action('wp_ajax_petshop_delete_pet', function() {
    global $wpdb;
    $id = intval($_POST['id']);
    $r = $wpdb->delete($wpdb->prefix.'petshop_pets', ['id'=>$id]);
    if ($r) wp_send_json_success();
    else wp_send_json_error();
});