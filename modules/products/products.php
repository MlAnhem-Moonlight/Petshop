<div class="wrap petshop-products-wrap">
    ...
    <div style="margin-bottom: 16px;">
        <label for="ps-products-per-page">Hiển thị:</label>
        <select id="ps-products-per-page" style="padding:4px 8px;">
            <option value="10" <?php selected($_GET['per_page'] ?? 10, 10); ?>>10</option>
            <option value="15" <?php selected($_GET['per_page'] ?? 10, 15); ?>>15</option>
    ...
    $total_products = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}petshop_products");
    ...
    <div class="ps-pagination" style="margin:18px 0;text-align:center;">
        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <?php
            $query = $_GET;
            $query['paged'] = $i;
            $query['per_page'] = $per_page;
            $url = admin_url('admin.php?page=ps-products') . '&' . http_build_query($query);
            ?>
            <a href="<?php echo esc_url($url); ?>" class="ps-page-link<?php if ($i == $page) echo ' active'; ?>" style="margin:0 3px;padding:6px 13px;border-radius:4px;<?php if ($i == $page) echo 'background:#8BC34A;color:#fff;'; else echo 'background:#f1f8e9;color:#333;'; ?>text-decoration:none;">
                <?php echo $i; ?>
            </a>
        <?php endfor; ?>
    </div>
...
jQuery(document).ready(function($) {
    ...
    $('#ps-products-per-page').on('change', function() {
